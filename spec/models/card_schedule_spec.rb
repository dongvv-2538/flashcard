...
RSpec.describe CardSchedule, type: :model do
  ...
  describe "scopes" do
    ...
    before do
      ...
      create(:card_schedule, card: card3, next_review_date: Date.today + 1, review_count: 1)
      # New card (never reviewed, next_review_date today)
      create(:card_schedule, card: card4, next_review_date: Date.today, review_count: 0)
    end

    describe ".due_today" do
      it "includes cards scheduled for today" do
        expect(CardSchedule.due_today).to include(CardSchedule.find_by(card: card1))
      end

      it "includes overdue cards (scheduled before today)" do
        expect(CardSchedule.due_today).to include(CardSchedule.find_by(card: card2))
      end

      it "excludes cards scheduled for the future" do
        expect(CardSchedule.due_today).not_to include(CardSchedule.find_by(card: card3))
      end
    end

    describe ".overdue" do
      it "includes cards scheduled before today" do
        expect(CardSchedule.overdue).to include(CardSchedule.find_by(card: card2))
      end

      it "excludes cards scheduled for today" do
        expect(CardSchedule.overdue).not_to include(CardSchedule.find_by(card: card1))
      end

      it "excludes future cards" do
        expect(CardSchedule.overdue).not_to include(CardSchedule.find_by(card: card3))
      end
    end

    describe ".new_cards" do
      it "includes cards with review_count of 0" do
        expect(CardSchedule.new_cards).to include(CardSchedule.find_by(card: card4))
      end

      it "excludes cards that have been reviewed" do
        expect(CardSchedule.new_cards).not_to include(CardSchedule.find_by(card: card1))
      end
    end
  end
end
