...
RSpec.describe StudySession, type: :model do
  ...
  describe "enums" do
    it "defines full_deck and review_due session types" do
      expect(StudySession.session_types).to include("full_deck" => 0, "review_due" => 1)
    end

    it "defaults to full_deck session type" do
      user = build(:user)
      deck = build(:deck, user: user)
      session = StudySession.new(user: user, deck: deck)
  ...
  describe "scopes" do
    describe ".completed" do
      it "returns sessions that have an ended_at timestamp" do
        ...
        expect(StudySession.completed).not_to include(incomplete)
      end
      ...
      it "returns sessions where cards_reviewed_count is positive" do
        ...
        expect(StudySession.completed).to include(reviewed)
        expect(StudySession.completed).not_to include(zero_review)
      end
    end
  end
