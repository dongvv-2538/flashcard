...
RSpec.describe SessionRating, type: :model do
  ...
  describe 'enums' do
    it 'defines again, hard, good, easy ratings' do
      expect(described_class.ratings).to include(
        'again' => 0,
        'hard' => 1,
        'good' => 2,
        'easy' => 3
  ...
  describe 'scopes' do
    describe '.by_rating' do
      it 'filters records by a given rating' do
        ...
        expect(described_class.where(rating: :good)).to include(good_rating)
        expect(described_class.where(rating: :good)).not_to include(again_rating)
      end
    end
  end
end
