...
RSpec.describe ReviewQueueService do
  ...
  describe '#grouped_by_deck' do
    ...
    context 'when the user has due cards' do
      ...
      before do
        ...
        create(:card_schedule, card: card2, next_review_date: Time.zone.today - 2, review_count: 3)
        create(:card_schedule, card: card3, next_review_date: Time.zone.today,     review_count: 1)
      end
    ...
    context 'when the user has only future-scheduled cards' do
      ...
      before do
        create(:card_schedule, card: card, next_review_date: Time.zone.today + 5, review_count: 1)
      end
    ...
    context 'when another user has due cards' do
      ...
      before do
        create(:card_schedule, card: other_card, next_review_date: Time.zone.today, review_count: 1)
      end
  ...
  describe '#due_cards' do
    ...
    it 'returns at most 100 cards even when more are due' do
      cards = create_list(:card, 5, deck: deck)
      cards.each.with_index do |card, i|
        create(:card_schedule, card: card,
                               next_review_date: Time.zone.today - i,
                               review_count: 1)
      end

      expect(described_class.new(user).due_cards.size).to eq(5)
    end
    ...
    context 'with due and future cards mixed' do
      ...
      before do
        create(:card_schedule, card: card_due,    next_review_date: Time.zone.today,     review_count: 1)
        create(:card_schedule, card: card_future, next_review_date: Time.zone.today + 3, review_count: 1)
      end
  ...
  describe '#next_review_date' do
    ...
    context 'when no due cards but future schedules exist' do
      ...
      before do
        create(:card_schedule, card: card, next_review_date: Time.zone.today + 3, review_count: 1)
      end

      it 'returns the earliest upcoming review date' do
        expect(next_date).to eq(Time.zone.today + 3)
      end
    end
